...
export default class Attributory extends React.Component {
  ...
  getEntityData = async (guid) => {
    ...
    const [events, nrAttribs]  =
      ...
      : (domain === 'BROWSER')
        ? [['PageAction', 'BrowserInteraction', 'PageView', 'AjaxRequest', 'BrowserTiming', 'JavaScriptError'], NRAttributes.Browser]
        : (domain === 'MOBILE')
          ? [['MobileSession', 'MobileCrash', 'MobileRequestError', 'MobileRequest', 'Mobile', 'MobileHandledException'], NRAttributes.Mobile]
          : [[], null];

    if (!events.length) return console.error('This type of entity is not supported in Attributory!');
  ...
  getAttributes = async () => {
    ...
    const nrqlArray = entityEvents.map(e => `${e.name}: nrql(query: "SELECT keySet() FROM ${e.name} WHERE entityGuid = '${entityGuid}' ${timePeriod} LIMIT MAX") {results}`);
    ...
    const dataStore = await EntityStorageQuery.query({ entityGuid: entityGuid, collection: 'attributoryDB' });
    const storedData = (dataStore || {}).data || [];
    const stored = storedData.reduce((acc, cur) => {
      const keys = cur.id.split(':');
      if  (keys.length > 1 && 'document' in cur && 'description' in cur.document) {
    ...
    const attributes = entityEvents.reduce((acc, evt) => {
      let results = (evt.name in keys && 'results' in keys[evt.name])
                    ...
                    : [];
      if (results.length && !(results.length === 1 && results[0].key === 'timestamp')) acc[evt.name] = results;
      return acc;
    }, {});
  ...
  closeEdit = (data, description) => {
    ...
    if (data) attributes[data.eventName] = attributes[data.eventName].map(attr => {
      if (attr.key === data.attributeName) attr.description = description || '';
      return attr;
    });
      ...
      <div style={{backgroundColor: 'white', padding: '1em'}}>
        ...
        <div className="filters">
          <div className="text-filter">
            ...
            <input type="text" placeholder="search" value={searchText} className="search" style={searchStyle} onChange={this.searchFor}/>
          </div>
          <div className="events-filter">
            {entityEvents && entityEvents.map(e => (
              <a href="#" key={'filter-'+e.name} className={'' + ((e.display) ? 'on' : 'off')} onClick={(evt) => this.filterEvent(evt, e.name)}>{e.name}</a>
            ))}
          </div>
        </div>
          <div className="attribs-info"><span className="tag nr">New Relic</span> marked attributes are built-in New Relic attributes</div>
          {entityEvents && attributes && entityEvents.map(e => <Event event={e.name} show={e.display} attributes={attributes[e.name] || []} searchText={searchText} editHandler={this.showEdit} key={e.name} />)}
          {editModal && editModal.mounted && <Modal hidden={editModal.hidden} onClose={this.closeEdit} onHideEnd={this.unmountEdit}>
